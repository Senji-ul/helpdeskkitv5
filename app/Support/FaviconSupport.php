...
abstract class FaviconSupport
{
    ...
    {
        $manifest = config('helpdeskkit.favicon.manifest');
...
<browserconfig>
    ...
    {
        return response(Vite::content(config('helpdeskkit.logo')), 200, ['Content-Type' => 'image/png']);
    }

    private static function getFavicon(): Response
    {
        return response(Vite::content(config('helpdeskkit.favicon')), 200, ['Content-Type' => 'image/x-icon']);
    }
}
